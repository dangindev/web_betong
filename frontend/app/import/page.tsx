...
const options = ["customers", "project_sites", "vehicles", "materials"];
...
export default function ImportPage() {
  ...
  const [resource, setResource] = useState(options[0]);
  ...
  if (!accessToken) {
    return <p className="text-sm text-slate-600">Bạn cần đăng nhập để import dữ liệu.</p>;
  }
    ...
    <div className="space-y-4">
      ...
      <form className="space-y-3 rounded border border-slate-200 bg-white p-4" onSubmit={handleImport}>
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="secondary" onClick={handlePreview} disabled={!file || busyPreview || busyImport}>
            {busyPreview ? "Đang preview..." : "Preview dữ liệu"}
          </Button>
          <Button type="submit" disabled={!file || busyImport}>
            {busyImport ? "Đang import..." : "Import valid rows"}
          </Button>
        </div>
      </form>
      ...
      {preview ? (
        <div className="space-y-3 rounded border border-slate-200 bg-white p-4">
          ...
          <div className="grid gap-3 text-sm md:grid-cols-4">
            <div className="rounded bg-slate-50 p-3">Total rows: {preview.total_rows}</div>
            <div className="rounded bg-green-50 p-3 text-green-700">Valid rows: {preview.valid_rows}</div>
            <div className="rounded bg-red-50 p-3 text-red-700">Invalid rows: {preview.invalid_rows}</div>
            <div className="rounded bg-blue-50 p-3 text-blue-700">Resource: {preview.resource}</div>
          </div>

          <div className="space-y-2">
            <p className="font-medium">Preview valid rows (tối đa 20):</p>
          ...
          <div className="space-y-2">
            <p className="font-medium">Error rows:</p>
            {preview.errors.length === 0 ? (
              ...
              <div className="overflow-auto rounded border border-slate-200">
                <table className="min-w-full text-sm">
                  <thead className="bg-slate-100">
                    <tr>
                      <th className="border-b border-slate-200 px-3 py-2 text-left font-semibold">Row #</th>
                      <th className="border-b border-slate-200 px-3 py-2 text-left font-semibold">Error</th>
                      <th className="border-b border-slate-200 px-3 py-2 text-left font-semibold">Data</th>
                    </tr>
                  </thead>
                  <tbody>
      ...
      {imported ? (
        <div className="rounded border border-green-200 bg-green-50 p-3 text-sm text-green-700">
          Import xong: created={imported.created}, skipped={imported.skipped}, valid_rows={imported.valid_rows}, invalid_rows={imported.invalid_rows}
        </div>
      ) : null}
    </div>
  );
