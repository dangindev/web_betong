...
- [x] Ghi nhận batch ticket actual vs mix design.
- [x] CostingEngine: direct_material, direct_labor, utilities, maintenance, depreciation, overhead allocation.
...
- [x] OR-Tools scheduler v2 (VRPTW + capacity + resource).
- [x] Learning loop: so sánh estimated vs actual, cập nhật travel_estimates và score weights.
- [x] Materialized views BI (mục 4.10) + refresh job.
**Checklist frontend**:
...
- [x] Toggle giữa scheduler v1/v2 + so sánh KPI.
**Test**:
...
- [x] Batch ticket actual vs mix design + CostingEngine breakdown groups (2026-04-20): endpoint /api/v1/costing/batch-tickets/actuals + /api/v1/costing/batch-ticket-variance đã có integration/guardrail/permission test; unit cost snapshot trả snapshot_json.cost_breakdown và snapshot_json.cost_breakdown_pct (backend 19 passed, frontend 19 passed).
- [x] Scheduler v1/v2 compare + learning loop + BI materialized views (2026-04-20): backend test suite 21 passed, gồm test_phase5_scheduler_v2_compare_and_learning_loop và test_phase5_bi_materialized_views_refresh_and_query; endpoint /api/v1/dispatch/scheduler-kpi-compare và /api/v1/costing/bi/materialized-views* trả dữ liệu đúng.
- [x] Frontend dispatch board toggle/compare (2026-04-20): trang /dispatch/board hỗ trợ chọn scheduler_mode v1/v2 và hiển thị bảng delta KPI; frontend test suite 17 passed, typecheck/lint/build pass (lint còn warnings hook deps).
**Acceptance**:
...
**Checklist (scope 1 web production, bỏ clone-site/staging theo yêu cầu):**
- [x] Rà soát NFR hiệu năng, availability, security cho phạm vi single production web.
- [x] Bổ sung hardening HTTP: backend security headers (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`) và frontend CSP/security headers.
- [x] Bổ sung rate limit API (global + login) qua middleware; smoke xác nhận `/api/v1/auth/login` trả `429` ở request thứ 121.
- [x] Hoàn tất UAT điều hướng/hierarchy UI: Báo giá nằm đúng phân cấp Điều hành kinh doanh; tách list/detail thành route riêng; mục Khung năng lực trạm đã bấm được và có trang riêng.
- [x] Verify regression backend: `pytest backend/tests/test_suite.py` -> 21 passed.
- [x] Verify regression frontend: `npm run typecheck` pass, `npm run test` 17 passed, `npm run build` pass, `npm run lint` chỉ còn warning hook deps (không block).
- [x] Verify runtime sau deploy: các route `/dieu-phoi/hang-cho-tram/khung-nang-luc-tram`, `/kinh-doanh/bang-gia/danh-sach`, `/kinh-doanh/bang-gia/quy-tac/:id`, `/quan-tri/tai-khoan/:id`, `/cau-hinh-he-thong/:id` trả HTTP 200.
- [x] Bỏ các hạng mục hardening/UAT liên quan clone-site/staging/đa-site; giữ 1 web production theo yêu cầu.
**Acceptance**:
- Baseline hardening đã bật ở runtime (CSP + security headers + rate limit).
- UAT smoke cho luồng Sales/Dispatch/Admin liên quan thay đổi hierarchy và route mới đã xanh.
- [Chưa xác minh] UAT đầy đủ end-to-end cho toàn bộ role nghiệp vụ ở môi trường vận hành thực tế cần chạy thêm với người dùng thật.
