...
- [ ] CI pipeline xanh trên nhánh chính.
**Acceptance**:
...
**Scope**: FR-02, FR-03.
**Checklist backend**:
- [ ] Migration nhóm pricing & sales (4.4).
- [ ] PricingEngine: rule types BasePrice, DistanceFee, DifficultyFee, PumpFee, Surcharge, Discount.
- [ ] Rule condition+formula schema (JSONB) với evaluator an toàn (không eval tùy ý code).
- [ ] Priority resolution, scope resolution theo region/customer/plant.
- [ ] Snapshot `price_calculation_snapshots` khi chốt.
- [ ] API `POST /pricing/preview`, CRUD price_books, price_rules, quotations, sales_orders, pour_requests, time_windows.
- [ ] Approval workflow discount/manual override.
- [ ] Import Excel price rules, pour_requests hàng loạt.
- [ ] Export PDF báo giá (WeasyPrint template).
**Checklist frontend**:
- [ ] Price book editor (mục 5.2).
- [ ] Quotation builder với pricing preview realtime.
- [ ] Pour request wizard 4 bước + quick create.
- [ ] Trang list sales order / pour request có filter theo trạng thái, customer, plant, ngày.
- [ ] Badge cảnh báo dữ liệu thiếu.
- [ ] PDF preview báo giá.
**Test**:
...
- [ ] Migration nhóm dispatch & scheduling (4.5).
- [ ] Migration nhóm trip execution & actuals (4.6).
- [ ] API duyệt đơn: approve/reject/request-info + dispatch_orders CRUD.
- [ ] Vehicle/pump availability service, resource_locks.
- [ ] Plant capacity slot service.
- [ ] Travel estimates adapter + cache 30 phút.
- [ ] Scheduler v1 heuristic (mục 5.5 công thức + mục 9.5 spec).
- [ ] Scheduler API: `POST /schedule-runs`, `GET /schedule-runs/{id}`, `GET /schedule-runs/{id}/conflicts`.
- [ ] Schedule version history khi chỉnh tay.
- [ ] Manual override API + audit before/after.
- [ ] WebSocket/SSE cho dispatch board realtime.
**Checklist backend - execution**:
- [ ] Trip state machine (assigned → accepted → check_in_plant → load_start → load_end → depart_plant → arrive_site → pour_start → pour_end → leave_site → return_plant).
- [ ] Event intake API idempotent theo `idempotency_key`.
- [ ] Pump session state machine.
- [ ] Batch ticket sinh khi load_start.
- [ ] GPS ping intake (rate limit per vehicle).
- [ ] Offline sync endpoint (batch apply).
- [ ] Reconciliation API: chốt actual volume/trip count, reason code.
- [ ] KPI worker: on-time, cycle time, utilization, empty km, trips/day.
- [ ] Notification adapter (Zalo/SMS/Email) + templates.
- [ ] Export báo cáo vận hành Excel/PDF.
**Checklist frontend**:
- [ ] Dispatcher inbox (mục 5.4).
- [ ] Dispatch board Gantt (mục 5.6, 6.8).
- [ ] Station queue board cho plant operator (mục 5.7).
- [ ] Driver mobile PWA (mục 5.8) với offline sync.
- [ ] Pump crew mobile PWA (mục 5.9) với signature pad.
- [ ] Reconciliation screen cuối ca.
- [ ] KPI dashboard ops manager (mục 5.10).
...
**Checklist backend**:
- [ ] Migration nhóm inventory (4.7).
- [ ] Migration cost_centers, cost_objects, cost_periods (4.9).
- [ ] Inventory ledger service append-only, balance calculation.
- [ ] Nghiệp vụ: receipt, issue, transfer, adjustment, waste, stock take.
- [ ] Validate qty khả dụng trước khi xuất/chuyển.
- [ ] Period open/close/reopen workflow.
- [ ] Pre-close checklist (chứng từ đủ, production đủ).
**Checklist frontend**:
- [ ] Warehouse management (mục 5.14).
- [ ] Form nhập, xuất, transfer, adjustment, stock take.
- [ ] Trang tồn kho hiện tại + drill-down transaction.
- [ ] Trang cost center (tree view), cost object, period calendar.
- [ ] Import phiếu nhập từ Excel.
- [ ] Export snapshot tồn kho.
**Test**:
- Unit inventory ledger + balance.
- Integration nhập/xuất/chuyển/adjust/stock take.
- Regression: không thể sửa ending balance.
- Integration open/close period + audit.
**Acceptance**:
...
**Checklist backend**:
- [ ] Migration nhóm production (4.8).
- [ ] Migration cost pools, allocation, unit cost snapshot, margin (4.9).
- [ ] Ghi nhận crushing (input, output, runtime, downtime, electricity).
...
- [ ] Allocation rule engine (basis: sản lượng, runtime, tỷ lệ).
- [ ] Close period: run allocation → snapshot unit cost → freeze.
- [ ] Margin snapshot theo đơn.
...
**Checklist frontend**:
- [ ] Form ghi nhận sản xuất đá/bê tông (mục 5.15).
- [ ] Close period wizard (mục 5.16).
- [ ] Preview unit cost + variance kỳ trước.
- [ ] Allocation rule editor.
- [ ] Dashboard Finance margin (mục 5.17).
